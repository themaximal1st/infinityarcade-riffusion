...
export default function SpectrogramViewer({
    ...
    const imageScaler = window.innerHeight / 3.4;
    const boxScaler = window.innerHeight / 3.76;
    const spectrogramImageScale =
        window.innerWidth > 767 && window.innerWidth < 1350
            ? window.innerWidth / imageScaler
            : 5;
    const spectrogramBoxScale =
        window.innerWidth > 767 && window.innerWidth < 1350
            ? window.innerWidth / boxScaler
            : 5.5;
      ...
      >
        <shaderPass attachArray="passes" args={[VerticalTiltShiftShader]} />
            ...
            {inferenceResults.map((result: InferenceResult, index: number) => {
                ...
                const position =
                    duration_s * (-0.55 - result.counter) + playbarShift;
